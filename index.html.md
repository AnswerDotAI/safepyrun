...
Python 3.12 and newer it also uses `sys.monitoring` to raise events for
calls into native extension modules, which would otherwise be invisible
to audit hooks (popular audited libraries such as numpy are declared
safe via the `fastaudit_safe_native` entry point and skipped).
...
You can also trust a whole package with a string prefix key:
`allow({'mypkg.*': ...})` permits any otherwise-denied operation that
happens inside a function defined in `mypkg` or its submodules. The
`...` value means full trust, so reserve it for packages you are happy
to let do anything. Named registrations and `(name, policy)` tuples
matching the same call stack are checked first, and a policy that raises
`PermissionError` denies the operation even though the blanket entry
matches, so one call of a broadly-trusted package can still be gated.
...
or, for popular audited libraries (numpy, pandas’ core, matplotlib,
`regex`, `orjson`, and others), is declared safe via fastaudit’s
`fastaudit_safe_native` entry point and works with no registration. That
entry point is what lets the
...
helpers stay narrow: their compute runs on the strength of the entry
point, and the helpers themselves only register save methods behind
write policies, so every destination is checked against `ok_dests`.
