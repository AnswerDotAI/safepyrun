...
Docs: https://AnswerDotAI.github.io/safepyrun/core.html.md"""

# AUTOGENERATED! DO NOT EDIT! File to edit: ../nbs/00_core.ipynb.

# %% auto #0
__all__ = ['mon_disable_policy', 'mon', 'allow_imports', 'default_ok_dests', 'find_var', 'freeze_mon_policy', 'on_call',
           'frame_args', 'RawDenyInfo', 'CallInfo', 'DenyInfo', 'before_deny', 'srcfn', 'set_data', 'RunPython',
...
mon_disable_policy = dict(callees=set(), callers=set(), pairs=set(), preds=(),
    callee_prefixes=(), callee_suffixes=('.dumps','.loads'), caller_prefixes=(), caller_suffixes=())
...
# %% ../nbs/00_core.ipynb #86d2fe4c
_data_defaults = {}

def set_data(**kw):
    "Set default entries for the per-run policy `data` dict; per-`RunPython` kwargs win"
    sys.audit('safepyrun.set_data', kw)
    _data_defaults.update(kw)
...
def _freeze(o):
    "Hashable snapshot of a policy value, for fingerprinting"
    if isinstance(o, (set,frozenset)): return frozenset(map(_freeze, o))
    if isinstance(o, (list,tuple)): return tuple(map(_freeze, o))
    if isinstance(o, dict): return frozenset((k,_freeze(v)) for k,v in o.items())
    try:
        hash(o)
        return o
...
def _check_user_code(tree, ban_imports, ban_defs):
    ...
    def banned(name): return any(name == b or name.startswith(b+'.') for b in ban_imports)
    for n in ast.walk(tree):
        ...
        if isinstance(n, ast.Import):
            for a in n.names:
                if banned(a.name): raise PermissionError(f"import {a.name!r} not allowed")
        elif isinstance(n, ast.ImportFrom):
            if n.module and banned(n.module): raise PermissionError(f"from {n.module!r} import not allowed")
...
_cfg_py.parent.mkdir(parents=True, exist_ok=True)
try:
    with _cfg_py.open('x') as _cfg_file: _cfg_file.write("default_ok_dests = ['.', '/tmp']\n")
